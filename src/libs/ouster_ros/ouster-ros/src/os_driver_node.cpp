...
class OusterDriver : public OusterSensor {
   public:
    ...
    explicit OusterDriver(const rclcpp::NodeOptions& options)
        ...
        declare_parameter("lidar_to_sensor_tf_x", 0.0);
        declare_parameter("lidar_to_sensor_tf_y", 0.0);
        declare_parameter("lidar_to_sensor_tf_z", 0.0);
        declare_parameter("lidar_to_sensor_tf_theta", 0.0);
    }
    ...
    virtual void create_publishers() override {
        ...
        Eigen::Matrix<double, 4, 4, Eigen::DontAlign> tf;
        double theta = get_parameter("lidar_to_sensor_tf_theta").as_double() * M_PI / 180.0;
        tf << cos(theta), -sin(theta), 0.0, get_parameter("lidar_to_sensor_tf_x").as_double(),
              sin(theta), cos(theta),  0.0, get_parameter("lidar_to_sensor_tf_y").as_double(),
              0.0,        0.0,         1.0, get_parameter("lidar_to_sensor_tf_z").as_double(),
              0.0,        0.0,         0.0, 1.0;
        info.lidar_to_sensor_transform = tf;
