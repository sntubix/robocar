...
namespace robocar
{
	namespace control
	{
		...
		ControlComponent::ControlComponent(const robocar::Params &params) : robocar::Component(params)
		{
			// params
		...
		void ControlComponent::serve()
		{
			...
			uint64_t stamp = robocar::utils::ros_to_unix_ms_time(_loc.header.stamp);
			...
			// init actuation
			msg::ActCmd act;
			act.header.stamp = robocar::utils::unix_ms_to_ros_time(stamp);
