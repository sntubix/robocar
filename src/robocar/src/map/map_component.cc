...
namespace robocar
{
	namespace map
	{
		...
		MapComponent::MapComponent(const robocar::Params &params) : robocar::Component(params)
		{
			// params
		...
		msg::Path MapComponent::load_waypoints(const std::string &filename)
		{
			...
			for (int i = 0; i < static_cast<int>(features.Size()); ++i)
			{
				...
				if (id.IsInt())
					...
					index = -robocar::utils::string2number<int>(id.GetString());
				else
				...
				else if (properties.HasMember("Field_4"))
				{
					...
					velocity = robocar::utils::string2number<double>(speed_str) * 0.44704;
				}
				else
				...
				if (velocity == 0.0)
					RCLCPP_WARN(this->get_logger(), "velocity limit is 0 for waypoint: %d", i);
			...
			fs.close();
			RCLCPP_INFO(this->get_logger(), "waypoints loaded: %zu", trace.waypoints.size());
