...
OusterComponent::OusterComponent(const robocar::Params &params) : robocar::Component(params)
{
    // params
    ...
    // connect to lidar
    RCLCPP_INFO(this->get_logger(), "connecting sensor at '%s'..", _src_ip.c_str());
    ...
    {
        RCLCPP_ERROR(this->get_logger(), "unable to connect sensor at '%s'", _src_ip.c_str());
    }
}
    ...
    if (_sensor_connected)
    {
        ...
        while (this->is_running())
        {
            ...
            if (st == ouster::sensor::TIMEOUT)
            {
                ...
                if (_nb_timeouts > 5)
                {
                    RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "sensor timeout, trying to reconnect..");
            ...
            {
                RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "sensor client returned error state, trying to reconnect..");
                _sensor_connected = false;
                break;
            }
            ...
            if (st & ouster::sensor::LIDAR_DATA)
            {
                ...
                {
                    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "failed to read a packet of the expected size, dropping current scan");
                    break;
                }
                ...
                if (scan_batcher(_packet_buf.get(), scan))
                {
                    ...
                    if (index != -1)
                    {
                        ...
                        cloud->header.stamp = robocar::Time::now().us();
