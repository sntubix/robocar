...
#define JOYSTICK_AXIS_BRAKE 2
    ...
    int JoystickComponent::joystick_get_num_devices()
    {
        ...
        if (joystick != NULL)
        {
            ...
            if (num_joysticks < 0)
            {
                RCLCPP_ERROR(this->get_logger(), "SDL_NumJoysticks - %s", SDL_GetError());
    ...
    JoystickComponent::JoystickComponent(const robocar::Params &params) : robocar::Component(params)
    {
        // params
        ...
        if (joystick == NULL)
        {
            ...
            if (init_result < 0)
            {
                RCLCPP_ERROR(this->get_logger(), "SDL_Init - %s", SDL_GetError());
                ret = JOYSTICK_ERROR;
            }
        }
        ...
        else
        {
            ...
            if (num_joysticks > 0)
            {
                ...
                if (ret == JOYSTICK_OK)
                {
                    auto msg = "found " + std::to_string(num_joysticks) + " devices -- connecting to device at system index " + std::to_string(device_index) + " - GUID: " + std::string(joystick_guid.ascii_string);
        ...
        RCLCPP_INFO(this->get_logger(), "waiting for joystick controls to zero");
        ...
        while (ret != JOYSTICK_ERROR)
        {
            ...
            {
                RCLCPP_INFO(this->get_logger(), "joystick controls successfully initialized");
    ...
    int JoystickComponent::joystick_update()
    {
        ...
        if (joystick != NULL)
        {
            if (joystick->controller != JOYSTICK_DEVICE_CONTROLLER_INVALID)
            {
                ...
                {
                    RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "SDL_GameControllerGetAttached - device not attached");
                }
                else
                {
                    ret = JOYSTICK_OK;
