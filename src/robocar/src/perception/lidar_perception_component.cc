...
LidarPerceptionComponent::LidarPerceptionComponent(const robocar::Params &params) : robocar::Component(params)
{
	// params
	...
	// subscriber
	_sub_point_cloud = this->create_subscription<msg::PointCloud>("sensors/lidar/points",
																  rclcpp::SensorDataQoS().keep_last(1),
																  std::bind(&LidarPerceptionComponent::on_point_cloud,
																			this, std::placeholders::_1));
}

void LidarPerceptionComponent::serve()
{
	// do nothing for now
}

void LidarPerceptionComponent::on_point_cloud(msg::PointCloud r_pc)
{
	// convert to pcl
	...
	for (auto &cluster : cloud_clusters)
	{
		...
		if (z_max - z_min >= _obj_min_height)
		{
			...
			object.header.stamp = robocar::utils::unix_ms_to_ros_time(stamp);
			...
			{
				msg::Vector3d point;
				point.x = (bbox[j][0] * cos_theta) - (bbox[j][1] * sin_theta) + object.ground_center.x;
				point.y = (bbox[j][0] * sin_theta) + (bbox[j][1] * cos_theta) + object.ground_center.y;
