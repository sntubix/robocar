    ...
    public:
        ...
        void register_module(const std::string name) {
            auto m_cons = std::function(
                [](const Params& params) { return std::shared_ptr<Module>{new T(params)}; }
            );
        ...
        void register_module(const std::string name, N arg) {
            auto m_cons = std::function(
                [arg](const Params& params) { return std::shared_ptr<Module>{new T(params, arg)}; }
            );
			_m_cons_map.insert({name, m_cons});
        }
        ...
        void register_service(const std::string name) {
            auto s_cons = std::function(
                [](const Params& params) { return std::shared_ptr<Service>{new T(params)}; }
            );
        ...
        void spin() {
            ...
            // start executor
            _spin_thread.reset(new std::thread([&]() { _executor->spin(); }));
        }
