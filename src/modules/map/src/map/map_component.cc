...
void MapComponent::serve() {
	...
	msg::Path waypoints;
	{
		...
		do {
			...
			if (((edist >= _waypoints_interdistance) || (waypoints.waypoints.size() == 0))
				&& (edist <= _waypoints_interdistance_max)) {
	...
	for (int i=0; i < (int(waypoints.waypoints.size()) - 1); i++) {
		...
		double x_v = std::cos(loc.yaw) * waypoint.x
				   + std::sin(loc.yaw) * waypoint.y
				   - std::cos(loc.yaw) * loc.x
				   - std::sin(loc.yaw) * loc.y;
		if (x_v <= 0.0) {
			continue;
		}
