...
MappingComponent::MappingComponent(const cycle::Params& params) : cycle::Service(params) {
	// params
...
double MappingComponent::compute_dist(const msg::GNSS& gnss_a, const msg::GNSS& gnss_b) {
	...
	// haversine formula
	double sqrt = std::sqrt(std::pow(sin((lat_b - lat_a) / 2.0), 2.0)
				  + cos(lat_a) * cos(lat_b) * std::pow(sin((lon_b - lon_a) / 2.0), 2.0));
...
void MappingComponent::on_toggle(const msg::Mapping& mapping) {
	if ((_mapping) && (!mapping.status)) {
		...
		LOG_INFO("saved map file to '" + _filename + "'");
	}
