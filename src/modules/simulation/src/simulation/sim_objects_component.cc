...
SimObjectsComponent::SimObjectsComponent(const cycle::Params& params) : cycle::Service(params) {
    // params
...
void SimObjectsComponent::serve() {
    ...
    uint64_t now = cycle::Time::now().ms();
    ...
    for (auto& agent : _agents) {
        ...
        obj.header.stamp = cycle::utils::unix_ms_to_ros_time(now);
    ...
    for (msg::Object3d& object : objects.objects) {
        ...
        // direction
        double dir_x = object.direction.x * cos_theta
                       - object.direction.y * sin_theta;
        double dir_y = object.direction.x * sin_theta
                       + object.direction.y * cos_theta;
		...
		// ground center
        double gc_x = cos_theta * object.ground_center.x
                      + sin_theta * object.ground_center.y
                      - cos_theta * _loc.x - sin_theta * _loc.y;
        double gc_y = -sin_theta * object.ground_center.x
                      + cos_theta * object.ground_center.y
                      + sin_theta * _loc.x - cos_theta * _loc.y;
		object.ground_center.x = gc_x;
		object.ground_center.y = gc_y;

		// bbox points
        for (msg::Vector3d& point : object.points) {
            double x = cos_theta * point.x + sin_theta * point.y
                       - cos_theta * _loc.x - sin_theta * _loc.y;
            double y = -sin_theta * point.x + cos_theta * point.y
                       + sin_theta * _loc.x - cos_theta * _loc.y;
			point.x = x;
			point.y = y;
        }
