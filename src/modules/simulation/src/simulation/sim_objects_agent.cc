...
SimObjectAgent::SimObjectAgent(double dim_x, double dim_y, double dim_z,
	...
	// header
	_object.header.stamp = cycle::utils::unix_ms_to_ros_time(0);
    ...
    for (int j=0; j < _bbox.size(); j++) {
        msg::Vector3d point;
        point.x = (_bbox[j][0] * cos_theta) - (_bbox[j][1] * sin_theta)
                  + _object.ground_center.x;
        point.y = (_bbox[j][0] * sin_theta) + (_bbox[j][1] * cos_theta)
                  + _object.ground_center.y;
...
msg::Object3d SimObjectClose::update(msg::Localization loc, msg::Path waypoints, double dt) {
    ...
    if (_state == 0) {
        if (waypoints.waypoints.size() > 0) {
            ...
            _object.ground_center.x = waypoints.waypoints[index_o].x
                                      + _offset_y * cos(pos_yaw);
            _object.ground_center.y = waypoints.waypoints[index_o].y
                                      + _offset_y * sin(pos_yaw);
...
msg::Object3d SimObjectStatic::update(msg::Localization loc, msg::Path waypoints, double dt) {
    ...
    double gc_x = cos_theta * _object.ground_center.x
                  + sin_theta * _object.ground_center.y
    ...
    if (((gc_x < -5.0) || (dist >= _max_dist)) && (waypoints.waypoints.size() > 0)) {
        ...
        _object.ground_center.x = waypoint.x + _offset_x * cos(theta)
                                  + _offset_y * cos(theta + M_PI_2);
        _object.ground_center.y = waypoint.y + _offset_x * sin(theta)
                                  + _offset_y * sin(theta + M_PI_2);
