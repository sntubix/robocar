...
void ControlComponent::serve() {
	...
	if (t.waypoints.size() > 0) {
		...
		for (auto& waypoint : t.waypoints) {
			double d = std::sqrt((waypoint.x * waypoint.x)
		...
		for (int i=0; i < t.waypoints.size(); i++) {
			...
			// euclidean distance
			dist = std::sqrt((t.waypoints[i].x * t.waypoints[i].x)
							 + (t.waypoints[i].y * t.waypoints[i].y));
		...
		if (target_point.accel > 0.0) {
			throttle = (_velocity_kp_a * target_point.accel)
						  + (_velocity_ki_a * _velocity_sum_e)
						  + (_velocity_kd_a * velocity_dt_e);
		}
		else {
			throttle = (_velocity_kp_b * target_point.accel)
						  + (_velocity_ki_b * _velocity_sum_e)
						  + (_velocity_kd_b * velocity_dt_e);
		}
		...
		safe_dist = _rss_min_dist + (v_r * _rss_reaction_time)
					+ (0.5 * _rss_max_accel * _rss_reaction_time * _rss_reaction_time)
					+ (std::pow(v_r + _rss_reaction_time * _rss_max_accel, 2.0) / (2 * _rss_min_brake))
					- ((v_f * v_f) / (2 * _rss_max_brake));
		if (safe_dist < 0.0) {
			safe_dist = 0.0;
		}
		...
		if (safe_dist_e < 0.0) {
			...
			// safe distance PID
			throttle = (_safe_dist_kp * safe_dist_e)
					   + (_safe_dist_ki * _safe_dist_sum_e)
					   + (_safe_dist_kd * safe_dist_dt_e);
		...
		if ((_th_filter && ((stamp - _th_filter_stamp) > _th_filter_duration))
			|| (safe_dist_e < 0.0)) {
			_th_filter = false;
		}
		...
		if ((obstacle_d >= 0.0) && (obstacle_d < _rss_min_dist)
			&& (curr_vel < _stop_velocity)) {
			target_velocity = 0.0;
...
msg::Trajectory ControlComponent::transform(msg::Trajectory const& t, msg::Localization const& loc) {
	...
	for (auto& waypoint : t.waypoints) {
		double x = cos_theta * waypoint.x + sin_theta * waypoint.y
				   - cos_theta * loc.x - sin_theta * loc.y;
		if (x <= 0.0) {
			continue;
		}

		double y = -sin_theta * waypoint.x + cos_theta * waypoint.y
