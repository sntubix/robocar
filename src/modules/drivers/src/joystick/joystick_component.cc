...
JoystickComponent::JoystickComponent(const cycle::Params& params) : cycle::Service(params) {
    ...
    else
    {
        ...
        if ( num_joysticks > 0 )
        {
            ...
            if ( ret == JOYSTICK_OK )
            {
                LOG_INFO("Found " + std::to_string(num_joysticks)
                       + " devices -- connecting to device at system index " + std::to_string(device_index) 
                       + " - GUID: " + std::string(joystick_guid.ascii_string));
...
void JoystickComponent::serve() {
    if (joystick_update() == JOYSTICK_OK) {
        ...
        // publish actuation
        if ((act.steering != _act.steering)
            || (act.throttle != _act.throttle)
            || (act.brake != _act.brake)) {
            _act = act;
