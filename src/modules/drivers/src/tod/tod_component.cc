...
using namespace robocar::drivers::tod;
...
TODComponent::TODComponent(const cycle::Params &params) : cycle::Service(params)
{
    // params
...
void TODComponent::on_gnss(const msg::GNSS &gnss)
{
	...
	_vehicle_data.stamp = cycle::utils::ros_to_unix_ms_time(gnss.header.stamp);
...
void TODComponent::on_tod_msg(const std::string& tod_msg)
{
    ...
    if (!doc.Parse(tod_msg.c_str()).HasParseError()) {
        ...
        msg::ActCmd act_cmd;
        act_cmd.header.stamp = cycle::utils::unix_ms_to_ros_time(input_stamp);
        act_cmd.mode = mode;
