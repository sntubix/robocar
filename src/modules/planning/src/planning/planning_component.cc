...
PlanningComponent::PlanningComponent(const cycle::Params& params) : cycle::Service(params) {
    // params
    ...
    _prev_planning.header.stamp = cycle::utils::unix_ms_to_ros_time(0);
    ...
    _prev_tfl.header.stamp = cycle::utils::unix_ms_to_ros_time(0);
    ...
    _tfl_input.header.stamp = cycle::utils::unix_ms_to_ros_time(0);
...
void PlanningComponent::serve() {
    ...
    double v_tfl_x = cos_theta * tfl.ground_center.x + sin_theta * tfl.ground_center.y
                     - cos_theta * loc.x - sin_theta * loc.y;
    double v_tfl_y = -sin_theta * tfl.ground_center.x + cos_theta * tfl.ground_center.y
                     + sin_theta * loc.x - cos_theta * loc.y;
...
msg::Object3d PlanningComponent::traffic_light_match(uint64_t now,
    ...
    for (auto& obj : objects.objects) {
        bool relevant = (tfl_type != OBJ_NONE) && ((obj.type == OBJ_TFL_GREEN)
                                                   || (obj.type == OBJ_TFL_YELLOW)
                                                   || (obj.type == OBJ_TFL_RED));
        bool match = (tfl_right && (obj.x1 >= last_img_x))
                     || (!tfl_right && (obj.x1 <= last_img_x));
        if (relevant && match) {
            last_img_x = obj.x1;
            tfl_stamp = obj_stamp;
            tfl_type = obj.type;
    ...
    if (((tfl_input.type == OBJ_TFL_GREEN)
         || (tfl_input.type == OBJ_TFL_YELLOW)
         || (tfl_input.type == OBJ_TFL_RED)) && (tfl_type != OBJ_NONE)) {
        tfl_stamp = now;
...
msg::Object3d PlanningComponent::traffic_light_fsm(const msg::Localization& loc,
    ...
    auto time_diff = ros_to_unix_ms_time(n_tfl.header.stamp)
                     - ros_to_unix_ms_time(p_tfl.header.stamp);
...
std::vector<float> PlanningComponent::get_object_circles(const msg::Localization& loc,
    ...
    for (auto& object : objects.objects) {
        ...
        double d = 0.0;
        double c_x = object.ground_center.x - (max_dim / 2.0) * cos(theta)
                     + edge_l * cos(theta);
        double c_y = object.ground_center.y - (max_dim / 2.0) * sin(theta)
                     + edge_l * sin(theta);
        d += edge_l;
        ...
        if (d >= max_dim) {
            ...
            c_x = object.ground_center.x + (max_dim / 2.0) * cos(theta)
                  - edge_l * cos(theta);
            c_y = object.ground_center.y + (max_dim / 2.0) * sin(theta)
                  - edge_l * sin(theta);
